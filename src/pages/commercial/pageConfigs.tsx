...
import { applyPayable, approvePayable, confirmBill, fetchBillItems, fetchBills, fetchCashFlows, fetchPayablePayments, fetchPayables, fetchProfits, parseBill, registerPayablePayment, uploadBill } from '../../api/fms'
...
import { batchCreateSkus, createSpu, fetchCategories, fetchSkuDetail, fetchSkus, fetchSpuDetail, fetchSpus, publishSpu, updateSkuPrice, updateSpu } from '../../api/pim'

import { auditPurchaseRequisition, cancelPurchaseOrder, comparePurchaseInquiry, confirmPurchaseOrder, confirmPurchaseReceipt, createPurchaseInquiry, createPurchaseOrder, createPurchaseReceipt, createPurchaseRequisition, fetchPurchaseInquiries, fetchPurchaseOrderDetail, fetchPurchaseOrders, fetchPurchaseReceipts, fetchPurchaseRequisitionDetail, fetchPurchaseRequisitions, markPurchaseOrderShipping, reconcilePurchaseOrder, selectPurchaseInquiry, submitPurchaseRequisition, urgePurchaseOrder } from '../../api/pms'
...
import { fetchSystemUsers } from '../../api/system'

import { cancelWaybill, createLogisticsChannel, createWaybill, disableLogisticsChannel, fetchLogisticsCarriers, fetchLogisticsChannels, fetchWaybillTracks, fetchWaybills, refreshWaybillTracks, updateWaybillStatus } from '../../api/tms'
...
const listQueryPath = (path: string, keyword?: string) => `${path}${keyword && keyword !== '-' ? `?keyword=${encodeURIComponent(keyword)}` : ''}`



const actions = (items: Array<{ label: string; danger?: boolean; hidden?: boolean; permission?: string; onClick?: () => void; to?: string }>) => {

  const visibleItems = items.filter((item) => !item.hidden)

  if (!visibleItems.length) return <span className="commercial-action-empty" />

  return (

    <Space className="commercial-action-group" size={6} wrap>

      {visibleItems.map((item) => {

        const className = item.danger ? 'commercial-action-button commercial-action-danger' : 'commercial-action-button'

        return item.to ? (

          <PermissionButton key={item.label} size="small" className={className} danger={item.danger} permission={item.permission} href={item.to}>
          ...
          <PermissionButton key={item.label} size="small" className={className} danger={item.danger} permission={item.permission} onClick={item.onClick}>
...
const currencyOptions: FormOption[] = [{ label: 'CNY', value: 'CNY' }, { label: 'USD', value: 'USD' }, { label: 'EUR', value: 'EUR' }]

const yesNoOptions: FormOption[] = [{ label: '否', value: 0 }, { label: '是', value: 1 }]



function stringifySpecValues(value: unknown) {

  if (!Array.isArray(value)) return value ? String(value) : undefined
  ...
  const entries = value
    ...
    .map((item) => {
      ...
      return [String(row.key || '').trim(), String(row.value || '').trim()] as const
...
async function loadSupplierOptions(): Promise<FormOption[]> {

  const page = await fetchSuppliers({ pageNum: 1, pageSize: 50, status: 1 })
  ...
  return page.records.map((item) => ({
    ...
    extra: { supplierName: item.supplierName },
...
async function loadWarehouseOptions(): Promise<FormOption[]> {

  const page = await fetchWarehouses({ pageNum: 1, pageSize: 50 })
  ...
  return page.records.map((item) => ({
    ...
    extra: { warehouseName: item.warehouseName, warehouseId: item.id },
...
async function loadSkuOptions(): Promise<FormOption[]> {

  const page = await fetchSkus({ pageNum: 1, pageSize: 50 })
...
async function loadSpuOptions(): Promise<FormOption[]> {

  const page = await fetchSpus({ pageNum: 1, pageSize: 50 })
...
async function loadCategoryOptions(): Promise<FormOption[]> {

  const categories = await fetchCategories()

  const flatten = (items: Array<Record<string, unknown>>, prefix = ''): FormOption[] =>

    items.flatMap((item) => {

      const id = String(item.id || '')

      const name = String(item.name || item.categoryName || item.menuName || id)

      const label = prefix ? `${prefix} / ${name}` : name

      const children = Array.isArray(item.children) ? (item.children as Array<Record<string, unknown>>) : []

      return [{ label, value: id, extra: { categoryPath: label } }, ...flatten(children, label)]

    })

  return flatten(categories as unknown as Array<Record<string, unknown>>)

}



async function loadRequisitionOptions(): Promise<FormOption[]> {

  const page = await fetchPurchaseRequisitions({ pageNum: 1, pageSize: 50 })
...
async function loadOrderOptions(): Promise<FormOption[]> {

  const page = await fetchOrders({ pageNum: 1, pageSize: 50 })

  return page.records.map((item) => ({

    label: `${item.orderNo} (${item.platformOrderNo})`,
...
async function loadChannelOptions(): Promise<FormOption[]> {

  const page = await fetchLogisticsChannels({ pageNum: 1, pageSize: 50, status: 1 })

  return page.records.map((item) => ({

    label: `${item.channelName} (${item.carrierName})`,
...
async function loadCarrierOptions(): Promise<FormOption[]> {

  const page = await fetchLogisticsCarriers({ pageNum: 1, pageSize: 50 })
...
async function loadUserOptions(): Promise<FormOption[]> {

  const page = await fetchSystemUsers({ pageNum: 1, pageSize: 50, status: 1 })
  ...
  return page.records.map((item) => ({
    ...
    extra: { operatorName: item.realName || item.username },
...
async function openSupplierEditForm(row: Supplier) {
  ...
  showFormDrawer(
    ...
    [
      ...
      { name: 'currency', label: '结算币种', type: 'select', options: currencyOptions },
...
function openCreatePurchaseOrderForm() {
  ...
  showFormDrawer(
    ...
    [
      ...
      { name: 'supplierId', label: '供应商', type: 'remote-select', required: true, loadOptions: loadSupplierOptions, fillFields: { supplierName: 'supplierName' } },

      { name: 'supplierName', label: '供应商名称', required: true },

      { name: 'warehouseId', label: '收货仓库', type: 'remote-select', required: true, loadOptions: loadWarehouseOptions, fillFields: { warehouseName: 'warehouseName' } },

      { name: 'warehouseName', label: '仓库名称', required: true },

      { name: 'currency', label: '币种', type: 'select', options: currencyOptions },
      ...
      { name: 'skuId', label: '采购 SKU', type: 'remote-select', required: true, loadOptions: loadSkuOptions, fillFields: { skuCode: 'skuCode', skuName: 'skuName', spec: 'spec', unit: 'unit', unitPrice: 'refPrice' } },
...
function openCreateRequisitionForm() {
  ...
  showFormDrawer(
    ...
    [
      ...
      { name: 'warehouseId', label: '需求仓库', type: 'remote-select', required: true, loadOptions: loadWarehouseOptions },
      ...
      { name: 'applyUserId', label: '申请人 ID', type: 'hidden', required: true },

      { name: 'applyUserName', label: '申请人姓名', required: true },

      { name: 'skuId', label: '申请 SKU', type: 'remote-select', required: true, loadOptions: loadSkuOptions, fillFields: { skuCode: 'skuCode', skuName: 'skuName', spec: 'spec', unit: 'unit', refPrice: 'refPrice' } },
...
function openCreateInquiryForm() {
  ...
  showFormDrawer(
    ...
    [
      ...
      { name: 'reqId', label: '关联采购申请', type: 'remote-select', loadOptions: loadRequisitionOptions },

      { name: 'supplierId', label: '供应商', type: 'remote-select', required: true, loadOptions: loadSupplierOptions, fillFields: { supplierName: 'supplierName' } },

      { name: 'supplierName', label: '供应商名称', required: true },

      { name: 'quoteDeadline', label: '报价截止时间' },

      { name: 'skuId', label: '询价 SKU', type: 'remote-select', required: true, loadOptions: loadSkuOptions, fillFields: { skuCode: 'skuCode', skuName: 'skuName', spec: 'spec', unit: 'unit', refPrice: 'refPrice' } },
    ...
    (values) =>
      ...
      createPurchaseInquiry({
        ...
        reqId: values.reqId === undefined ? undefined : Number(values.reqId),

        supplierId: Number(values.supplierId),

        supplierName: String(values.supplierName),

        quoteDeadline: values.quoteDeadline ? String(values.quoteDeadline) : undefined,
        ...
        items: [

          {
            ...
            refPrice: values.refPrice === undefined ? undefined : Number(values.refPrice),
    ...
    { unit: '件', applyUserId: 1, applyUserName: '当前用户' },
...
function openPurchaseOrderReceiptForm(row: PurchaseOrder) {
  ...
  showFormDrawer(
    ...
    [
      ...
      { name: 'receiverId', label: '收货人 ID', type: 'hidden', required: true },
...
function openPayablePaymentForm(row: Payable) {

  const remainingAmount = Math.max(Number(row.amount || 0) - Number(row.paidAmount || 0), 0)

  showFormDrawer(

    `登记应付单 ${row.payableNo} 付款`,

    [

      { name: 'paymentAmount', label: '付款金额', type: 'number', required: true },

      { name: 'paymentDate', label: '付款日期', required: true },

      { name: 'paymentMethod', label: '付款方式', type: 'select', required: true, options: [{ label: '银行转账', value: 1 }, { label: '线上支付', value: 2 }, { label: '现金', value: 3 }] },

      { name: 'voucherNo', label: '付款凭证号' },

      { name: 'operatorId', label: '经办人', type: 'remote-select', loadOptions: loadUserOptions, fillFields: { operatorName: 'operatorName' } },

      { name: 'operatorName', label: '经办人姓名', type: 'hidden' },
    ...
    (values) => {
      ...
      if (paymentAmount <= 0) throw new Error('付款金额必须大于 0')

      if (paymentAmount > remainingAmount) throw new Error('付款金额不能超过剩余应付金额')

      return registerPayablePayment(row.id, {
        ...
        operatorId: values.operatorId === undefined ? 1 : Number(values.operatorId),

        operatorName: values.operatorName ? String(values.operatorName) : '当前用户',
...
function openSpuEditForm(row: ProductSpu) {
  ...
  showFormDrawer(
    ...
    [
      ...
      { name: 'categoryId', label: '商品分类', type: 'remote-select', required: true, loadOptions: loadCategoryOptions, fillFields: { categoryPath: 'categoryPath' } },
...
function openBatchSkuForm() {
  ...
  showFormDrawer(
    ...
    [
      ...
      { name: 'spuId', label: '所属 SPU', type: 'remote-select', required: true, loadOptions: loadSpuOptions },
      ...
      { name: 'specValues', label: '规格配置', type: 'spec-values' },
      ...
      { name: 'isBattery', label: '含电池', type: 'select', options: yesNoOptions },

      { name: 'isLiquid', label: '含液体', type: 'select', options: yesNoOptions },

      { name: 'isPowder', label: '含粉末', type: 'select', options: yesNoOptions },

      { name: 'costPrice', label: '成本价', type: 'number' },

      { name: 'costCurrency', label: '成本币种', type: 'select', options: currencyOptions },
    ...
    (values) =>
      ...
      batchCreateSkus(Number(values.spuId), [

        {
          ...
          specValues: stringifySpecValues(values.specValues),
    ...
    { isBattery: 0, isLiquid: 0, isPowder: 0, costCurrency: 'CNY', abcClass: 'C', status: 0, specValues: [{ key: '颜色', value: '' }, { key: '尺码', value: '' }] },
...
function openCreateStocktakeForm() {
  ...
  showFormDrawer(
    ...
    [
      ...
      { name: 'warehouseId', label: '盘点仓库', type: 'remote-select', required: true, loadOptions: loadWarehouseOptions, fillFields: { taskName: 'warehouseName' } },
  ...
  try {
    ...
    showFormDrawer(
      ...
      [
        ...
        { name: 'pickerId', label: '盘点人 ID', type: 'hidden', required: true },

        { name: 'items', label: '实盘明细', type: 'stocktake-items', required: true },

      ],

      (values) => {

        const parsedItems = Array.isArray(values.items) ? (values.items as Array<Record<string, unknown>>) : []
...
function confirmStocktakeAudit(row: StocktakeTask) {
  ...
  showFormDrawer(
    ...
    [
      ...
      { name: 'auditorId', label: '审核人 ID', type: 'hidden', required: true },
...
function openCreateChannelForm() {
  ...
  showFormDrawer(
    ...
    [
      ...
      { name: 'carrierId', label: '物流商', type: 'remote-select', required: true, loadOptions: loadCarrierOptions },
      ...
      { name: 'allowBattery', label: '是否允许电池', type: 'select', options: yesNoOptions },

      { name: 'allowLiquid', label: '是否允许液体', type: 'select', options: yesNoOptions },

      { name: 'allowPowder', label: '是否允许粉末', type: 'select', options: yesNoOptions },

      { name: 'allowFood', label: '是否允许食品', type: 'select', options: yesNoOptions },
...
function openWarehouseEditForm(row: Warehouse) {
  ...
  showFormDrawer(
    ...
    [
      ...
      { name: 'isDefault', label: '默认仓', type: 'select', options: yesNoOptions },
...
function openCreateWaybillForm() {
  ...
  showFormDrawer(
    ...
    [
      ...
      { name: 'orderId', label: '关联订单', type: 'remote-select', required: true, loadOptions: loadOrderOptions, fillFields: { orderNo: 'orderNo', warehouseId: 'warehouseId', countryCode: 'countryCode' } },

      { name: 'orderNo', label: '订单号', required: true },

      { name: 'warehouseId', label: '发货仓库', type: 'remote-select', required: true, loadOptions: loadWarehouseOptions },

      { name: 'channelId', label: '物流渠道', type: 'remote-select', loadOptions: loadChannelOptions },
      ...
      { name: 'declaredCurrency', label: '申报币种', type: 'select', options: currencyOptions },

      { name: 'declaredNameEn', label: '英文申报名', required: true },

      { name: 'hsCode', label: 'HS Code' },

      { name: 'isGift', label: '是否礼品', type: 'select', options: yesNoOptions },

      { name: 'hasBattery', label: '含电池', type: 'select', options: yesNoOptions },

      { name: 'hasLiquid', label: '含液体', type: 'select', options: yesNoOptions },

      { name: 'hasPowder', label: '含粉末', type: 'select', options: yesNoOptions },
...
export const pageConfigs = {
  ...
  skus: {
    ...
    columns: [
      ...
      { title: '操作', width: 210, fixed: 'right', render: (_, row) => actions([{ label: '查看', permission: 'pim:sku:edit', onClick: () => showRecordDrawer(`SKU ${row.skuCode} 详情`, row, () => fetchSkuDetail(row.id)) }, { label: '库存', permission: 'wms:inventory:list', to: listQueryPath('/wms/inventory', row.skuCode) }, { label: '价格', permission: 'pim:sku:price', onClick: () => openSkuPriceForm(row) }]) },
  ...
  inquiries: {
    ...
    onPrimaryAction: openCreateInquiryForm,
...

      {
        ...
        render: (_, row) =>
          ...
          actions([
            ...
            { label: '拣货', onClick: () => showRecordDrawer(`出库单 ${row.outboundNo} 拣货明细`, row, () => fetchOutboundPicklist(row.id)) },

            { label: '订单', permission: 'oms:order:list', to: listQueryPath('/oms/order', row.orderNo) },

            { label: '运单', permission: 'tms:waybill:list', to: listQueryPath('/tms/waybill', row.orderNo) },

            { label: '出库', hidden: !statusIn(row.status, ['待复核']), permission: 'wms:outbound:ship', onClick: () => confirmOutboundShip(row) },
...

      {
        ...
        render: (_, row) =>
          ...
          actions([
            ...
            { label: '出库单', permission: 'wms:outbound:list', to: listQueryPath('/wms/outbound', row.orderNo) },
...

      {
        ...
        render: (_, row) => {
          ...
          return actions([
            ...
            ...(canPay ? [{ label: '付款', permission: 'fms:payable:pay', onClick: () => openPayablePaymentForm(row) }] : []),
